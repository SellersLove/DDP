...
struct ParaLightListNode {
	ParaLightListNode() : paraLight(NULL), next(NULL) {}
	ParaLightListNode( ParaLightSource* paraLight, ParaLightListNode* next = NULL ) : 
		paraLight(paraLight), next(next) {}
	~ParaLightListNode() { 
		if (!paraLight) //delete paraLight;
		{
			free(paraLight);
		} 
	}
	ParaLightSource* paraLight;
	ParaLightListNode* next;
};
...
struct Camera {
	...
	Camera(): resolution_x((int)NULL), resolution_y((int)NULL) {}
	Camera(int x, int y): resolution_x(x), resolution_y(y){
...
public:
	...
	// Renders an image fileName with width and height 
	// with parallel light along view derection.
	
	void renderParallel( double width, double height, int resWidth, int resHeight, 
	...
	// Add a parallel light source.
	ParaLightListNode* addParaLightSource(ParaLightSource* paraLight);
...
private:
	...
	void projectionShading( Ray3D& ray );
	
	void projectionParaShading( Ray3D& ray );
	...
	ParaLightListNode *_paraLightSource;
	SceneDagNode *_root;
